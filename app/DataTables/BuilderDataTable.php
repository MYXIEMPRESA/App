...
class BuilderDataTable extends DataTable
{
    use DataTableTrait;
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('status', function($data) {
                $action_type = 'status';
                return view('builder.action',compact('data','action_type'))->render();
            })
            ...
            ->addColumn('action', function($builder){
                $id = $builder->id;
                $action_type = 'action';
                return view('builder.action',compact('action_type','id'))->render();
            })
            ->addIndexColumn()
    ...
    {
        $model = User::where('user_type','user')->where('is_builder',1);

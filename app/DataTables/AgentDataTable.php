...
class AgentDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('status', function($data) {
                $action_type = 'status';
                return view('agent.action',compact('data','action_type'))->render();
            })
            ...
            ->addColumn('action', function($agent){
                $id = $agent->id;
                $action_type = 'action';
                return view('agent.action',compact('action_type','id'))->render();
            })
            ->addIndexColumn()
    ...
    {
        $model = User::where('user_type','user')->where('is_agent',1);
